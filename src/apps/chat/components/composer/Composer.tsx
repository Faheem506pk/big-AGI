...
} from '~/common/components/speechrecognition/useSpeechRecognition';
...
import {
  ...
  duplicateDMessageFragmentsNoVoid,
...
export function Composer(props: {
  ...
  const _handleSendActionUnguarded = React.useCallback(
  ...
  const handleSendAction = React.useCallback(
    async (chatExecuteMode: ChatExecuteMode, composerText: string): Promise<boolean> => {
      setSendStarted(true);
  ...
  const onSpeechResultCallback = React.useCallback(
    (result: SpeechResult) => {
      ...
      if (autoSend) {
        ...
        if (result.flagSendOnDone) setSendStarted(false);
  ...
  const onActileCommandPaste = React.useCallback(
    ({ label }: ActileItem, searchPrefix: string) => {
    ...
    React.useMemo(() => {
      ...
      if (recognitionState.isActive) {
        composerShortcuts.push({
          key: 'm',
          ctrl: true,
          action: handleFinishMicAndSend,
          description: 'Mic · Send',
          disabled: !recognitionState.hasSpeech || sendStarted,
  ...
  let textPlaceholder: string = isDraw
    ? 'Describe what you would like to see...'
    : isReAct
      ? 'Ask a multi-step reasoning question...'
      : isTextBeam
        ? 'Combine insights from multiple AI models...'
        : showChatInReferenceTo
          ? 'Chat about this...'
          : 'Type' +
              ...
              >
                ...
                <Box sx={{ position: 'relative' /* for Mic overlay */ }}>
                  ...
                  {recognitionState.isAvailable && (
                    ...
                    >
                      {/* {isDesktop && <ButtonAttachFilesMemo onAttachFiles={handleAttachFiles} fullWidth multiple />} */}
                      {/* Responsive Paste button */}
                      {/* {supportsClipboardRead() && <ButtonAttachClipboardMemo onClick={attachAppendClipboardItems} />} */}
